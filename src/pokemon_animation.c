...
// counting from Id 1, because 0 in gSpeciesToBackAnimSet is used for mons with no back animation
...
{
    if (gSpeciesToBackAnimSet[species] != 0)
        return gSpeciesToBackAnimSet[species] - 1;
    else
        return 0;
}
